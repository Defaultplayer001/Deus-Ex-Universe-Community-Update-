...
Make a post exec for ANY exe in the root CU folder
if not x"%ValueSafeFileName%"==x"ModsCommunity Update" goto noexe
...
rem FileListFinal excluded to catch crappy demo post exec stuff
del "%~dp0\*Transcended*.txt"

rem Delete relatively weighty 100mb+ Muses folder so as not to waste the user's HD space :P
del "%~dp0\*Muses*FileListFinal.txt"

Handling demo group manually for crappy Transcended demo
del "%~dp0\*Demo*FileListFinal.tx
