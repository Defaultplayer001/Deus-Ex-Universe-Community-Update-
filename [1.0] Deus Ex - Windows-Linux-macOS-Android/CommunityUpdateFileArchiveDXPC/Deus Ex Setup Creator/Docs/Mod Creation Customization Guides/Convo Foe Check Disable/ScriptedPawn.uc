...
{
	if (bForce || (IsValidEnemy(newEnemy) && (Robot(newEnemy) == None || !Robot(newEnemy).IsInState('Disabled'))))
	{
		if (newEnemy != Enemy)
			EnemyTimer = 0;
		Enemy         = newEnemy;
                             ...
                             out Name CarcassName, optional bool bCheckName)
{
	...
	{
		if (carcass.bEmitCarcass && ((pp == None || (pp != None && pp.bAIReactUnconscious)) || !carcass.bNotDead))
		{
...
function Pawn CheckCycle()
{
	...
	else  // I saw somebody!
	{
		...
		if (EnemyReadiness >= 1.0)
		{
			...
			else if (TranscendedDecoration(CycleCandidate) != None)
			{
				HandleActorSighting(CycleCandidate);
				EnemyLastSeen = 0;
				TranscendedDecoration(CycleCandidate).UpdateLocation(CycleCandidate.Location);
				if (Lamp(cycleCandidate) != None)
					Lamp(cycleCandidate).LastSeenState = Lamp(cycleCandidate).bOn;
                                 ...
                                 bool bCheckPlayer, bool bCheckOther)
{
	...
	local bool         bCheck;
	local TranscendedDecoration  cycleDeco;
	...
	if (bReactPresence && bLookingForEnemy && !bNoNegativeAlliances)
	{
		...
		if (TranscendedPawn(Self) != None && TranscendedPawn(Self).bRecogniseMovedObjects && TranscendedPawn(Self).localPlayer != None && TranscendedPawn(Self).localPlayer.combatDifficulty >= 4 && Human(TranscendedPawn(Self).localPlayer).bAIRecogniseMovedObjects && cycleEnemy == None && GetPawnAllianceType(TranscendedPawn(Self).localPlayer) == ALLIANCE_Hostile /* && !IsInState('Seeking')*/)
		{
			// foreach CycleActors(Class'RevDecoHook', cycleDeco, CycleIndex)
			foreach VisibleCollidingActors(Class'TranscendedDecoration', cycleDeco, 2048,,True)
			{
				if (cycleDeco.bMemorable || cycleDeco.IsInState('Burning') || Lamp(cycleDeco) != None)
				{
					...
					if (visibility > 0)
					{
						...
						if (cycleDeco.IsInState('Burning') || (cycleDeco.origLocation != cycleDeco.Default.origLocation && VSize(cycleDeco.Location - cycleDeco.origLocation) > cycleDeco.CollisionRadius * 3) || (Lamp(cycleDeco) != None && Lamp(cycleDeco).bOn != Lamp(cycleDeco).LastSeenState))
						{
...
function bool IsProjectileDangerous(DeusExProjectile projectile)
{
	...
	if (projectile.IsA('Cloud') && !IsA('AnnaNavarre') && !IsA('GuntherHermann') && !IsA('PaulDenton') && !IsA('WaltonSimons') && !IsA('MJ12Commando') && !IsA('RiotCop'))
		bEvil = true;	// We don't take damage from gas, so don't run
	else if (projectile.IsA('ThrownProjectile'))
	{
		...
		else if (ThrownProjectile(projectile).IsA('GasGrenade') && (IsA('AnnaNavarre') || IsA('GuntherHermann') || IsA('PaulDenton') || IsA('WaltonSimons') || IsA('MJ12Commando') || IsA('RiotCop')))
			bEvil = false;	// We don't take damage from gas, so don't run
		else
			bEvil = true;
	}
	else
...
function DropWeapon()
{
	...
	if (Weapon != None)
	{
		...
		if ((dxWeapon == None) || !dxWeapon.bNativeAttack)
		{
			...
			if (DeusExWeapon(oldWeapon).Default.PickupAmmoCount == 0)
				DeusExWeapon(oldWeapon).PickupAmmoCount = 0;
			else if (DeusExWeapon(oldWeapon).Default.PickupAmmoCount <= 8)
				DeusExWeapon(oldWeapon).PickupAmmoCount = Rand(4) + 1;
			else
				DeusExWeapon(oldWeapon).PickupAmmoCount = 1 + Rand(DeusExWeapon(oldWeapon).Default.PickupAmmoCount) / 2;
		}
	}
}
...
function SetWeapon(Weapon newWeapon)
{
	...
	pp = Human(GetPlayerPawn());
	if ((pp != None && pp.bAINonStandardAmmo) || pp == None)
	{
		if (Weapon != None && Weapon.ReloadCount > 0 && DeusExWeapon(Weapon) != None)
		{
			...
			if (DeusExWeapon(Weapon).AmmoName.IsA('Ammo20mm'))
				DeusExWeapon(Weapon).bInstantHit = False;
			else if (DeusExWeapon(Weapon).AmmoName.IsA('Ammo762mm'))
				DeusExWeapon(Weapon).bInstantHit = DeusExWeapon(Weapon).Default.bInstantHit;
		}
	}
                            ...
                            Vector offset, Name damageType)
{
	...
	{
		if (((instigatedBy != None) && (VSize(instigatedBy.Location - Location) < 96)) && ((bStunned) || (offset.x < 0)))
		{
			actualDamage *= 7.0;
		...
		{/*
			...
			{*/
				...
				if ((damageType == 'Stunned') || (damageType == 'KnockedOut'))
					HealthHead -= actualDamage * 4; // Transcended - DO allow headshots with stunning weapons
					// HealthHead -= actualDamage;
				else
					HealthHead -= actualDamage * 6; // Transcended - reduced due to easier headshots.
			...
			{
				HealthTorso -= actualDamage * 2;	// Transcended - Cut
				if (offset.x < 0.0)
                        ...
                        bool bPlayAnim)
{
	...
	if (Health <= 0)
	{
		...
		{
			bBurnedToDeath = true;
			ScaleGlow *= 0.25;  // blacken the corpse
		}
		else
	...
	{
		if (TranscendedPawn(Self) != None && instigatedBy != None)
			TranscendedPawn(Self).Burner = instigatedBy;
		CatchFire();
	}
	...
	// Transcended - Added
	if (!IsA('Animal') && !IsA('Robot') && (HealthLegLeft < 1))
		GroundSpeed = Default.GroundSpeed * 0.85;
	else if (!IsA('Animal') && !IsA('Robot') && (HealthLegRight < 1))
		GroundSpeed = Default.GroundSpeed * 0.85;
	
	if (!IsA('Animal') && !IsA('Robot') && (HealthLegLeft < 1) && (HealthLegRight < 1))
		GroundSpeed = Default.GroundSpeed * 0.7;
}
...
function GotoDisabledState(name damageType, EHitLocation hitPos)
{
	if (!bCollideActors && !bBlockActors && !bBlockPlayers)
		...
		GotoState('Stunned');
	else if (IsInState('RubbingEyes') && (damageType == 'Poison' || damageType == 'PoisonEffect'))
		return;
...
function PlayIdleSound()
{
	...
	if (dxPlayer != None && (!HeadRegion.Zone.bWaterZone || (UnderWaterTime > 5000 || UnderWaterTime < 0))) // Transcended - Only talk if we can breathe underwater or we're not in water
...
function PlayTargetAcquiredSound()
{
	...
	dxPlayer = DeusExPlayer(GetPlayerPawn());
	if ((dxPlayer != None) && (Enemy == dxPlayer) && (!HeadRegion.Zone.bWaterZone || (UnderWaterTime > 5000 || UnderWaterTime < 0))) // Transcended - Only talk if we can breathe underwater or we're not in water
...
function PlayTargetLostSound()
{
	...
	dxPlayer = DeusExPlayer(GetPlayerPawn());
	if ((dxPlayer != None) && (SeekPawn == dxPlayer) && (!HeadRegion.Zone.bWaterZone || (UnderWaterTime > 5000 || UnderWaterTime < 0))) // Transcended - Only talk if we can breathe underwater or we're not in water
...
function PlayCriticalDamageSound()
{
	...
	dxPlayer = DeusExPlayer(GetPlayerPawn());
	if ((dxPlayer != None) && (Enemy == dxPlayer) && !IsInState('Stunned') && (!HeadRegion.Zone.bWaterZone || (UnderWaterTime > 5000 || UnderWaterTime < 0))) // Only talk if we can breathe underwater or we're not in water
...
function ForcePlaySurpriseSound()
{
	...
	dxPlayer = DeusExPlayer(GetPlayerPawn());
	if ((dxPlayer != None) && (!HeadRegion.Zone.bWaterZone || (UnderWaterTime > 5000 || UnderWaterTime < 0))) // Only talk if we can breathe underwater or we're not in water
		dxPlayer.StartAIBarkConversation(self, BM_Surprise);
}
...
function PlayFootStep()
{
	...
	// if (RevPawnHook(Self).captionWalking != "")
		// RevPawnHook(Self).ClosedCaptions("FOOTSTEP", self);
	...
	if (W != None)
	{
		if (Region.Zone.bWaterZone)
		{
			if (W.bHandToHand)
				...
				LoopAnimPivot('TreadShoot',0.5,0.1,0.5,GetSwimPivot()); // Transcended - Possible fix for SCUBA divers.
		}
		else
		{
			if (W.bHandToHand)
                            ...
                            optional vector extent, optional bool bIgnoreLevel)
{
	...
	// Transcended - No more magic numbers
	...
	{		
		...
		if (tracePawn != None)
		{
			if (tracePawn != self)
			{
				// Transcended - Don't shoot through potential allies
				if (GetPawnAllianceType(tracePawn) != ALLIANCE_Hostile)
				{
					if (Robot(tracePawn) == None || (Robot(tracePawn) != None && Robot(tracePawn).EMPHitPoints > 0)) // Transcended - Don't care about disabled robots being used as shields.
						bSafe = false;
				}
				break;
			}
		...
		if (traceMover != None)
		{
			...
			else if (DeusExWeapon(Weapon) != None && ((DeusExWeapon(Weapon).Default.HitDamage) < traceMover.minDamageThreshold))
			{
				bSafe = false;
				break;
			}
                         ...
                         optional float throwAccuracy, optional bool bDiscountMinRange)
{
	...
	if (!bIsThrown)
	{
		...
		// Transcended - Changed to have AI shoot through movers (the fast trace was seen as geo, not a mover).
		if (!bSafe)
                             ...
                             out float maxAccurateRange, out float maxRange)
{
	...
	if (pawnWeapon != None)
	{
		...
		projectileClass = Class<DeusExProjectile>(pawnWeapon.ProjectileClass);
...
//	if ( Target == None )
	...
	if (Target.IsA('Human')
	&& (Human(Target).CombatDifficulty >= 4) && Human(Target).bAISmartAiming) // Are we fighting the player who is playing on realistic?
	{
		...
		{
			if ((ExplosiveTarget.bExplosive || (Barrel1(ExplosiveTarget) != None && (Barrel1(ExplosiveTarget).SkinColor == SC_Biohazard || Barrel1(ExplosiveTarget).SkinColor == SC_Poison))) && !ExplosiveTarget.IsA('Van') && !ExplosiveTarget.bHidden && !ExplosiveTarget.bInvincible && (AICanSee(ExplosiveTarget, 1.0, false, false, false, true) > 0) && ExplosiveTarget.PlayerCanSeeMe()) // Is there an explosive that both we and the target have direct LOS to?
			{
...
*/
	if (TranscendedPawn(Self) != None && TranscendedPawn(Self).LaserCheckTimer > 0)
	{
		TranscendedPawn(Self).LaserCheckTimer -= deltaTime;
		if (TranscendedPawn(Self).LaserCheckTimer < 0)
			TranscendedPawn(Self).LaserCheckTimer = 0;
	}
/*
	if (FutzTimer > 0)
	{
...
*/
	...
	else
	{
		...
		{
			if (TranscendedPawn(Self) != None)	
				TranscendedPawn(Self).CheckLaserPresence(deltaTime);
	...
	while (inv != None)
	{
		...
		if (curWeapon != None)
		{
			...
			if (bValid)
			{
				...
				if (enemyPlayer != None && enemyPlayer.combatDifficulty >= 4 && Human(enemyPlayer).bAISmartWeaponSwap && curWeapon.ClipCount == curWeapon.ReloadCount)
					curFallbackLevel = 1;
...
function Reloading(DeusExWeapon reloadWeapon, float reloadTime)
{
	...
	if (pp != None && pp.CombatDifficulty >= 4 && pp.bAISmartWeaponSwap)
		SwitchToBestWeapon();
...
function Died(pawn Killer, name damageType, vector HitLocation)
{
	...
	// Transcended - Let's fire the grenade if the AI was holding one and the player is playing on realistic.
	if (Weapon != None)
	{
		if ((Weapon.IsA('WeaponLAM') || Weapon.IsA('WeaponGasGrenade') || Weapon.IsA('WeaponEMPGrenade') || Weapon.IsA('WeaponNanoVirus')) && (DeusExGameInfo(Level.Game) != None && DeusExGameInfo(Level.Game).Difficulty == 3 && Human(Player).bAIDropArmedGrenades))
		{
			DeusExWeapon(Weapon).ProjectileFire(DeusExWeapon(Weapon).ProjectileClass, 0, true);
			Weapon.Destroy();
		}
	}
}
...
function EnterConversationState(bool bFirstPerson, optional bool bAvoidState)
{
	...
	if ((GetStateName() == 'Conversation') || (GetStateName() == 'FirstPersonConversation') || (GetStateName() == 'Dying')) // Transcended - No longer while dying
		return;
...
state RubbingEyes
{
	...
	{
		// if ((damageType != 'TearGas') && (damageType != 'HalonGas') && (damageType != 'Stunned'))
		if ((damageType != 'TearGas') && (damageType != 'HalonGas')) // Transcended - Removed stunned
