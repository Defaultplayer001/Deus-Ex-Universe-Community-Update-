...
var bool		bBuySkills;
var bool 		bVocalize; // Is the vocalization menu open?
...
replication
{
    ...
    reliable if ((Role == ROLE_Authority) && (bNetOwner))
        AugmentationSystem, SkillSystem, SkillPointsTotal, SkillPointsAvail, inHand, inHandPending, KeyRing, 
		  bSpyDroneActive, DXGame, bBuySkills, drugEffectTimer, killProfile, bVocalize;

    reliable if (Role == ROLE_Authority)
       ShieldStatus, RunSilentValue, aDrone, NintendoImmunityTimeLeft, Energy;
	...
	reliable if (Role < ROLE_Authority)
      ...
      ActivateMedkit, ActivateBiocell, Vocalize, DXTVocalizeMedkit, DXTVocalizeThanks, DXTVocalizeRIP, DXTVocalizeFutz, DXTVocalizeThreaten, DXTVocalizeMoveUp, DXTVocalizePositive, DXTVocalizeNegative, DXTVocalizeYes, DXTVocalizeNo; 
...
function PreTravel()
{
	...
	// Transcended: Turn off the laser on the weapon before traveling to prevent lots of laser dots in the maps.
	ourWeapon = DeusExWeapon(Weapon);
	
	if (ourWeapon != None)
	{
		ourWeapon.bWasLasing = ourWeapon.bLasing;
		ourWeapon.LaserOff();
	}
}
...
event TravelPostAccept()
{
	...
	if (AugmentationSystem.NumAugsActive() == 0)
		AmbientSound = None;
}
...
exec function QuickSave()
{
   ...
   // 4) We're in a multiplayer game
	...
	QuickSaveNum = int(ConsoleCommand("get Human QuickSaveNumber"));
	...
	{
		case 0:
			ConsoleCommand("set Human QuickSaveNumber " $ -1);
			SaveGame(-1, QuickSaveGameTitle @ "(0)");
			break;
			
		case -1:
			ConsoleCommand("set Human QuickSaveNumber " $ -5);
			SaveGame(QuickSaveNum, QuickSaveGameTitle @ "(1)");
			break;
			
		case -5:
			ConsoleCommand("set Human QuickSaveNumber " $ -6);
...
function ResetPlayer(optional bool bTraining)
{
	...
	if (!bTraining)
	{
		...
		DeusExWeapon(anItem).bItemRefusalOverride = true;
		...
		DeusExPickUp(anItem).bItemRefusalOverride = true;
		anItem.Frob(Self, None);
		anItem.bInObjectBelt = True;
	}
}
...
function ResetPlayerToDefaults()
{
   ...
   // reset the image linked list
	...
	Human(Self).dripRate = 0;
	Human(Self).waterDripCounter = 0;
...
exec function AugAdd(class<Augmentation> aWantedAug)
{
	...
	if (AugmentationSystem != None)
	{
		anAug = AugmentationSystem.GivePlayerAugmentation(aWantedAug, True);
...
function int HealPlayer(int baseHealPoints, optional Bool bUseMedicineSkill)
{
	...
	if (adjustedHealAmount > 0)
	{
		...
		{
			// If legs broken, heal them a little bit first
...
//			Velocity.Z *= 0.9;
		...
		if ( Base != Level && Base != None && ThrownProjectile(Base) == None) // Hack
...
*/
	...
	event PlayerTick(float deltaTime)
	{
      ...
      // DEUS_EX AMSD For multiplayer...
		...
		// Disable Radius Check -DP
		// Or actually leave the convo start distance check enabled, to prevent it trying to load every convo on the map at once. You just need the mid convo one disabled.
		CheckActorDistances();
...
state PlayerSwimming
{
	...
	event PlayerTick(float deltaTime)
	{
      ...
      MultiplayerTick(deltaTime);
		...
		Human(Self).dripRate += deltaTime*2;
			
		if (Human(Self).dripRate >= 15)
			Human(Self).dripRate = 15.0;
	...
	function PlayerCalcView(out actor ViewActor, out vector CameraLocation, out rotator CameraRotation)
	{
		...
		if (bHidden)
		{
				  ...
				  ((killProfile != None) && killProfile.bValid && (!killProfile.bKilledSelf)))
			{
				...
				/*
				if (( !killProfile.bKilledSelf ) && ( Pawn(myKiller) != None ) && DeusExGameInfo(DXGame).bIsTeamGame())
				{
					CameraLocation = myKiller.Location + (Pawn(myKiller).BaseEyeHeight * vect(0,0,1));
					if (Pawn(myKiller).bIsPlayer && PlayerIsListenClient())
						CameraRotation = Pawn(myKiller).ViewRotation;
					else
						CameraRotation = Pawn(myKiller).Rotation;
					CameraLocation += myKiller.CollisionRadius * Vector(CameraRotation);
				}*/
			}
			else if (time < 8.0)
			{
...
{
	if ( Level.NetMode != NM_Standalone && !bVocalize )
	{
...
exec function ParseLeftClick()
{
	...
	if ((inHand != None) && !bInHandTransition)
	{
		if (inHand.bActivatable)
			inHand.Activate();
		else if (FrobTarget != None && !FrobTarget.bDeleteMe) // Transcended - Fix duping
		{
...
exec function ParseRightClick()
{
	...
	if (FrobTarget != None && (!FrobTarget.bDeleteMe || (Level.Game != None && Level.Game.Difficulty == 0)) && ((FrobTarget.IsA('DeusExMover') && DeusExMover(FrobTarget).bFrobbable) || !FrobTarget.IsA('DeusExMover'))) // If the mover isn't frobbable, don't try to frob it, just put our weapon away or drop the carcass
	{
		loc = FrobTarget.Location;
		
		if (Level.NetMode != NM_StandAlone && Medkit(inHand) != None && ((DeusExPlayer(FrobTarget) != None  && DeusExPlayer(FrobTarget).PlayerReplicationInfo.Team == PlayerReplicationInfo.Team) || (ScriptedPawn(FrobTarget) != None && Robot(FrobTarget) == None && ScriptedPawn(FrobTarget).GetPawnAllianceType(Self) != ALLIANCE_Hostile)))
		{
			HealTarget(Medkit(inHand).HealAmount, Pawn(FrobTarget), True);
			Medkit(inHand).UseOnce();
			return;
		}
		if (Level.NetMode != NM_StandAlone && BioelectricCell(inHand) != None && ((DeusExPlayer(FrobTarget) != None  && DeusExPlayer(FrobTarget).PlayerReplicationInfo.Team == PlayerReplicationInfo.Team)))
		{
			EnergizeTarget(BioelectricCell(inHand).rechargeAmount, DeusExPlayer(FrobTarget));
			BioelectricCell(inHand).UseOnce();
			return;
		}
...
function bool HandleItemPickup(Actor FrobTarget, optional bool bSearchOnly)
{
	...
	// Special checks for objects that do not require physical inventory
	...
	else if (FrobTarget.IsA('DeusExPickup'))
	{
		...
		if ((FindInventoryType(FrobTarget.Class) != None) && FrobTarget.IsA('Binoculars'))
		{
			ClientMessage(Sprintf(CanCarryOnlyOne, DeusExPickup(FrobTarget).itemName));
			bCanPickup = False;
		}
		else if ((FindInventoryType(FrobTarget.Class) != None) && (DeusExPickup(FrobTarget).bCanHaveMultipleCopies))
			bSlotSearchNeeded = False;
		else if (!DeusExPickup(FrobTarget).bItemRefusalOverride && Level.NetMode == NM_StandAlone)
		{
			if(GetItemRefusalSetting(DeusExPickUp(FrobTarget)))
			{
				ClientMessage(Sprintf(Human(Self).ItemRefusedString, DeusExPickup(FrobTarget).itemName));
				DeusExPickup(FrobTarget).bItemRefusalOverride = True;
				bCanPickup = False;
			}
		}
	}
	else 
	{
		...
		if (foundItem != None)
		{
			...
			else if (foundItem.IsA('Weapon'))
			{
				...
				//G-Flex: continue to disallow multiple melee weapon pickups
				if (bSlotSearchNeeded && DeusExWeapon(foundItem).bHandToHand)
					bCanPickup = false;
		...
		else if (!DeusExWeapon(FrobTarget).bItemRefusalOverride && Level.NetMode == NM_StandAlone)
		{
			if(GetItemRefusalSetting(DeusExWeapon(FrobTarget)))
			{
				ClientMessage(Sprintf(Human(Self).ItemRefusedString, DeusExWeapon(FrobTarget).itemName));
				DeusExWeapon(FrobTarget).bItemRefusalOverride = True;
				bCanPickup = False;
			}
		}
	}
	...
	if (bSlotSearchNeeded && bCanPickup)
	{
		...
		{
			ClientMessage(Sprintf(InventoryFull, Inventory(FrobTarget).itemName, Inventory(FrobTarget).invSlotsX, Inventory(FrobTarget).invSlotsY));
			bCanPickup = False;
	...
	{
		// If we don't have space for food, just use it immediately
		if (FrobTarget.IsA('Candybar') || FrobTarget.IsA('SodaCan') || FrobTarget.IsA('WineBottle') || FrobTarget.IsA('Liquor40oz') || FrobTarget.IsA('LiquorBottle') || FrobTarget.IsA('SoyFood'))
		{
...
function Bool FindInventorySlot(Inventory anItem, optional Bool bSearchOnly)
{
	...
	if (Level.NetMode == NM_Standalone)
	{
		if ((DeusExPickUp(anItem) != None && !DeusExPickUp(anItem).bItemRefusalOverride) || (DeusExWeapon(anItem) != None && !DeusExWeapon(anItem).bItemRefusalOverride))
			if (GetItemRefusalSetting(anItem))
				return False;
	}
...
function bool CanBeLifted(Decoration deco)
{
	...
	(deco.IsA('DeusExDecoration') && (DeusExDecoration(deco).GetStackMass(deco) > maxLift)) || 
	(!deco.IsA('DeusExDecoration') && ((deco.StandingCount > 0) || (deco.Mass > maxLift))))
	{
		if (deco.IsInState('Burning'))
			ClientMessage(Human(Self).TooHotToLift);
				...
				if (POVCorpse(item).carcClassString != "")
				{
					...
					if (carcClass != None)
					{
						...
						if (carc != None)
						{
							...
							//G-Flex: get familiar name as well
							carc.FamiliarName = POVCorpse(item).travelFamiliarName;
							carc.Tag = POVCorpse(item).travelTag;	// And the tag too
							carc.bBleedHead = POVCorpse(item).bBleedHead;
							...
							{
								carc.bHidden = True;
								carc.Destroy();	// Transcended - No longer spawns blood and stacks bodies on failure to place, such as vents.
							}

							//Bjorn: Force doing all the initialization again now that all properties have been set.
							carc.PostBeginPlay();
						}
					}
				}
...
exec function ShowLogsWindow()
{
	...
	if ((Level.NetMode != NM_Standalone) && (bBeltIsMPInventory))
	{
		...
		InvokeUIScreen(Class'PersonaScreenLogsMP');
		return;
	}
...
exec function ActivateBelt(int objectNum)
{
	...
	else if ((Level.NetMode != NM_Standalone) && bVocalize)
	{
		root = DeusExRootWindow(rootWindow);
		if ( root != None )
		{
			if ( root.hud.hvo.OverrideBelt( Self, objectNum ))
				return;
		}
	}
...
event PlayerCalcView( out actor ViewActor, out vector CameraLocation, out rotator CameraRotation )
{
	...
	if (bSpyDroneActive)
	{
		...
		{
			// First-person view.			
			if (Human(Self).bAltSpyDroneView) // Spydrone is big view, player view is small window.
			{
				CameraLocation = aDrone.Location;
				CameraRotation = aDrone.Rotation;
				bBehindView = True;	// Required to make JC appear in the main view.
			}
...
function EndConversation()
{
	...
	// Or actually disable it all for my silly mod -DP
   ...
   if (Level.NetMode != NM_Standalone)
	...
	if (( Level.NetMode == NM_DedicatedServer ) || ( Level.NetMode == NM_ListenServer ))
	{
		...
		else if ((MPHitLoc == 3) || (MPHitLoc == 4))	// Leg region
		{
			...
			if (MPHitLoc == 4)
			{
				if (bPlayAnim && HasAnim('HitLegRight'))
					PlayAnim('HitLegRight', , 0.1);
			}
			else if (MPHitLoc == 3)
			{
				if (bPlayAnim && HasAnim('HitLegLeft'))
		...
		else // arms and torso now one region
		{
			...
			if (MPHitLoc == 6)
			{
				if (bPlayAnim && HasAnim('HitArmRight'))
					PlayAnim('HitArmRight', , 0.1);
			}
			else if (MPHitLoc == 5)
			{
				if (bPlayAnim && HasAnim('HitArmLeft'))
					PlayAnim('HitArmLeft', , 0.1);
			}
			else
			{
				if (bPlayAnim && HasAnim('HitTorso'))
	...
	if ((offset.x < 0.0) && bPlayAnim)
	{
		...
		{
			// narrow the head region
			if (((Abs(offset.x) < headOffsetY) || (Abs(offset.y) < headOffsetY)) && HasAnim('HitHeadBack'))
				PlayAnim('HitHeadBack', , 0.1);
		}
		else if (HasAnim('HitTorsoBack'))
	...
	{
		if ((offset.x < 0.0) && bPlayAnim && HasAnim('WaterHitTorso'))
			PlayAnim('WaterHitTorsoBack',,0.1);
		else if (HasAnim('WaterHitTorso'))
	...
	if (CarriedDecoration != None)
	{
		if (AugmentationSystem != None && AugmentationSystem.GetClassLevel(class'AugMuscle') < 3)
			DropDecoration();
		else if (AugmentationSystem == None)
			DropDecoration();
	}
	...
	if ((DamageType == 'Flamed') && !bOnFire)
	{
		...
		if (UsingChargedPickup(class'HazMatSuit') && (skillLevel < 0.5))
		{
		}
		else if (augLevel > 0.2) // Prevent burning with a max level energy shield.
		{
			// Notify player if they're getting burned for the first time
			if ( Level.NetMode != NM_Standalone )
...
function bool DXReduceDamage(int Damage, name damageType, vector hitLocation, out int adjustedDamage, bool bCheckOnly)
{
	...
	if ((damageType == 'Shot') || (damageType == 'Sabot') || (damageType == 'Exploded') || (damageType == 'AutoShot'))
	{
		...
		if (Damage > 10 && ReducedDamageType != 'All')
		{
			if	(InHand != None && InHand.IsA('POVCorpse') && POVCorpse(InHand).bNotDead)
			{
				KillChance = Rand(35);
			
				if (KillChance < Damage)
					POVCorpse(InHand).bNotDead = False;
			}
		}
	}
...
exec function AllSkills()
{
	...
	{
		swimDuration = UnderWaterTime * SkillSystem.GetSkillLevelValue(class'SkillSwimming');
		swimTimer = (swimDuration - swimTimer);
		WaterSpeed = Default.WaterSpeed * SkillSystem.GetSkillLevelValue(class'SkillSwimming');
	}
}
		...
		do
		{
			...
			if (item.bDisplayableInv || item.IsA('Ammo'))
			{
				...
				if ((W != None) && W.bHandToHand && (W.ProjectileClass != None))
					strCopies = " ("$W.AmmoType.AmmoAmount@DeusExAmmo(W.AmmoType).AmmoRoundNameLong$")";
				else if (item.IsA('Ammo') && (Ammo(item).PickupViewMesh != Mesh'TestBox'))
					strCopies = " ("$Ammo(item).AmmoAmount@DeusExAmmo(item).AmmoRoundNameLong$")";
...
{
	if (Player.Console==None || Player.Console.Class!=ConsoleClass)
		FailConsoleCheck();
}
...
// ----------------------------------------------------------------------
function VerifyRootWindow(Class<RootWindow> RootWindowClass)
{
	if (RootWindow==None || RootWindow.Class!=RootWindowClass)
		FailRootWindowCheck();
}
...
singular function DripWater(float deltaTime)
{
	...
	// if (!DeusExGameInfo(Level.Game).bSpawnEffects)
	// {
		// Human(Self).dripRate = 0;
		// Human(Self).waterDripCounter = 0;
		// return;
	...
	// Copied from ScriptedPawn::Tick()
	Human(Self).dripRate = FClamp(Human(Self).dripRate, 0.0, 15.0);
	// Human(Self).dripRate = FClamp(Human(Self).dripRate, 0.0, 1.0);
	if (Human(Self).dripRate > 0)
	{
		// adjustedRate = (15.0-Human(Self).dripRate)*1.0+0.1;  // max 10 drops per second
		adjustedRate = 1;
		dripPeriod = adjustedRate / FClamp(VSize(Velocity)/512.0, 5, 10);
		Human(Self).waterDripCounter += deltaTime;
		while (Human(Self).waterDripCounter >= dripPeriod)
		{
			vel = 0.1*Velocity;
			vel.Z = 0;
			...
			waterVector = vect(0,0,1)*CollisionHeight*0.5;  // so folks don't drip from the crotch
			...
			Human(Self).waterDripCounter -= dripPeriod;
		}
		Human(Self).dripRate -= deltaTime;
	}
	if (Human(Self).dripRate <= 0)
	{
		Human(Self).dripRate   		 = 0;
...
exec function DXTVocalizeMedkit()
{
	...
	soundName = soundName $ randomSound;	
	soundName = "TranscendedSounds." $ soundName;
	
	soundToPlay = Sound(DynamicLoadObject(soundName, class'Sound'));
	
	if (Level.NetMode != NM_StandAlone && PlayerIsListenClient())
		PlayOwnedSound(soundToPlay, SLOT_Talk,,True);
	else
		PlaySound(soundToPlay, SLOT_Talk,,True);
}

exec function DXTVocalizeThanks()
{
	local int randomSound;
	local String soundName;
	local Sound soundToPlay;
...
exec function DXTVocalizeThreaten()
{
	local int randomSound;
	local String soundName;
	local Sound soundToPlay;
...
exec function DXTVocalizeMoveUp()
{
	local int randomSound;
	local String soundName;
	local Sound soundToPlay;
...
exec function DXTVocalizePositive()
{
	local int randomSound;
	local String soundName;
	local Sound soundToPlay;
...
exec function DXTVocalizeNegative()
{
	local int randomSound;
	local String soundName;
	local Sound soundToPlay;
...
exec function ActivateMedkit()
{	
	local Inventory Inv;
	
	Inv = FindInventoryType(Class'DeusEx.Medkit');
	if ( Inv != None )
		ClientMessage(Inv.ItemName @ Human(Self).CountRemainingString @ (Pickup(Inv).NumCopies - 1));
	
	ActivateInventoryItem(Class'DeusEx.Medkit');
}

exec function ActivateBioCell()
{
	local Inventory Inv;
	
	Inv = FindInventoryType(Class'DeusEx.BioelectricCell');
	if ( Inv != None )
		ClientMessage(Inv.ItemName @ Human(Self).CountRemainingString @ (Pickup(Inv).NumCopies - 1));
	
	ActivateInventoryItem(Class'DeusEx.BioelectricCell');
}

// ----------------------------------------------------------------------
// HealTarget()
// Used for healing targets, for pawns.
// ----------------------------------------------------------------------
function int HealTarget(int baseHealPoints, Pawn Target, optional Bool bUseMedicineSkill)
{
	local float mult;
	local int adjustedHealAmount, aha2, aha3, tempaha;
	local int origHealAmount;
	local float dividedHealAmount;
	...
	{
		if (bUseMedicineSkill)
			Target.PlaySound(sound'MedicalHiss', SLOT_None,,, 256);

		HealPart(Target.HealthHead, adjustedHealAmount);
		aha2 = adjustedHealAmount;
		aha3 = aha2;
		HealPart(Target.HealthTorso, adjustedHealAmount);
		if (Level.NetMode != NM_Standalone)
		{
			// aha2 = adjustedHealAmount;
			// HealPart(Target.HealthArmRight, aha2);
			// HealPart(Target.HealthArAmount;
			// Arms and torso are same in multiplayer
			HealPart(Target.HealthArmRight, aha2);
			HealPart(Target.HealthArmLeft, aha3);
		}
		else
		{
			HealPart(Target.HealthArmRight, adjustedHealAmount);
			HealPart(Target.HealthArmLeft, adjustedHealAmount);
		}
		if (Level.NetMode != NM_Standalone)
		{
			aha2 = adjustedHealAmount;
			HealPart(Target.HealthLegRight, aha2);
			HealPart(Target.HealthLegLeft, adjustedHealAmount);
		}
		else
		{
			HealPart(Target.HealthLegRight, adjustedHealAmount);
			HealPart(Target.HealthLegLeft, adjustedHealAmount);
		}

		if (ScriptedPawn(Target) != None)
		{
			ScriptedPawn(Target).PoisonCounter = ScriptedPawn(Target).PoisonCounter / mult;
			ScriptedPawn(Target).PoisonTimer = ScriptedPawn(Target).PoisonTimer / mult;
			ScriptedPawn(Target).UpdatePoison(deltaTime);
		}
		else if (DeusExPlayer(Target) != None)
		{
			if (Level.NetMode != NM_Standalone)
			{
				DeusExPlayer(Target).StopPoison();
				DeusExPlayer(Target).ExtinguishFire();
				DeusExPlayer(Target).drugEffectTimer = 0;
			}
			else
			{
				DeusExPlayer(Target).PoisonCounter = DeusExPlayer(Target).PoisonCounter / mult;
				DeusExPlayer(Target).PoisonTimer = DeusExPlayer(Target).PoisonTimer / mult;
				DeusExPlayer(Target).UpdatePoison(deltaTime);
			}
		}

		if (DeusExPlayer(Target) != None)
			DeusExPlayer(Target).GenerateTotalHealth();
		else if (ScriptedPawn(Target) != None)
			ScriptedPawn(Target).GenerateTotalHealth();
		...
		// {
			if (adjustedHealAmount == 1)
			{
				...
				{
					if (Target.bIsPlayer)
						Target.ClientMessage(PlayerReplicationInfo.PlayerName @ "healed" @ adjustedHealAmount $ " point" );
				}
				ClientMessage( "Healed" @ Target.PlayerReplicationInfo.PlayerName @ adjustedHealAmount $ " point");
			}
			else
			{
				if (Target != None)
				{
					if (Target.bIsPlayer)
						Target.ClientMessage(PlayerReplicationInfo.PlayerName @ "healed" @ adjustedHealAmount $ " points" );
				}
				ClientMessage( "Healed" @ Target.PlayerReplicationInfo.PlayerName @ adjustedHealAmount $ " points");
			}
		// }
		// else
		// {
...
// ----------------------------------------------------------------------
// EnergizeTarget()
// Used for energizing targets, for pawns.
// ----------------------------------------------------------------------
function int EnergizeTarget(int baseChargePoints, DeusExPlayer Target)
{
	local int chargedPoints;
	...
	chargedPoints = Min(Target.EnergyMax - Int(Target.Energy), baseChargePoints);

	Target.Energy += chargedPoints;
	...
	if (chargedPoints == 1)
	{
		...
		{
			if (Target.bIsPlayer)
				Target.ClientMessage(PlayerReplicationInfo.PlayerName @ "recharged" @ chargedPoints $ " point" );
		}
		ClientMessage( "Recharged" @ Target.PlayerReplicationInfo.PlayerName @ chargedPoints $ " point");
	}
	else
	{
		if (Target != None)
		{
			if (Target.bIsPlayer)
				Target.ClientMessage(PlayerReplicationInfo.PlayerName @ "recharged" @ chargedPoints $ " points" );
		}
		ClientMessage( "Recharged" @ Target.PlayerReplicationInfo.PlayerName @ chargedPoints $ " points");
	}
	
	if (Level.NetMode != NM_StandAlone && PlayerIsListenClient())
		PlayOwnedSound(sound'BioElectricHiss', SLOT_None,,, 256);
	else 
...
// ----------------------------------------------------------------------
// state CheatFlying.
// Transcended - Allows full control
// ----------------------------------------------------------------------
state CheatFlying
{
...
{
	BroadcastMessage( PlayerReplicationInfo.PlayerName @ "changed name to:" @ S );
	SetName(S);
}
...
defaultproperties
{
     ...
     InventoryFull="You don't have enough room in your inventory to pick up the %s, you require %dx%d"
